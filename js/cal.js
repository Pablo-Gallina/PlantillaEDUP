...
function days(x) {
  ...
  for (i = 0; i <= dayOfFirstDateOfMonth; i = i + 1) {if (window.CP.shouldStopExecution(0)) break;
    if (dayOfFirstDateOfMonth === 6) {break;}
    dayItems.innerHTML += "<li> - </li>";
  }window.CP.exitedLoop(0);
  for (counter = 1; counter <= lastDayOfMonth; counter = counter + 1) {if (window.CP.shouldStopExecution(1)) break;
